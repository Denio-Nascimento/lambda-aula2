...
def lambda_handler(event, context):
    ...
    try:
        ...
        logger.error(f"Objeto {object_key} não encontrado no bucket {bucket_name}: {e}")
        return {
            'statusCode': 404,
            'body': json.dumps(f"Arquivo {object_key} não encontrado no bucket {bucket_name}.")
        }

    except Exception as e:
        logger.error(f"Erro ao processar o arquivo {object_key} do bucket {bucket_name}: {e}")
        return {
            'statusCode': 500,
            'body': json.dumps(f"Erro ao processar o arquivo {object_key} do bucket {bucket_name}.")
        }
